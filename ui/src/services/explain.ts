...
const _GUARD_DESCRIPTIONS: Record<string, string> = {
    injection_guard:
        'Regex threat scoring with 8 injection patterns. Blocks "ignore previous instructions", role-play attacks, system prompt extraction before the request reaches the upstream.',
    language_guard:
    ...
    pii_masker:
        'Dual-mode PII detection: Presidio NLP if installed, regex fallback otherwise. Masks emails, phones, SSNs, credit cards, IBANs.',
};

async function _renderFirewall(): Promise<HTMLElement> {
    const data: GuardsStatus = await api.fetchGuardsStatus();
...
async function _renderGuard(name: string): Promise<HTMLElement> {
    const data: GuardsStatus = await api.fetchGuardsStatus();
...
async function _renderCircuit(endpointId: string): Promise<HTMLElement> {
    const data: GuardsStatus = await api.fetchGuardsStatus();
...
async function _renderEndpoint(endpointId: string): Promise<HTMLElement> {
...
async function _renderProviderCount(): Promise<HTMLElement> {
    const registry: EndpointLike[] = store.state.registry ?? [];
    ...
    const rows = Object.entries(bySource)
        .map(
            ([k, v]) =>
                `<div class="flex justify-between py-1"><span class="text-[11px] text-slate-400 font-mono">${k}</span><span class="text-[11px] text-white font-mono">${v}</span></div>`
        )
        .join('');
...
async function _buildContent(kind: string, id: string | null): Promise<HTMLElement> {
...
async function _open(kind: string, id: string | null): Promise<void> {
    ...
    try {
        ...
        if (handle.isOpen) handle.setBody(_errorNode((e as Error)?.message ?? String(e)));
    }
}
...
function _attachGlobalHandler(): void {
    ...
    document.addEventListener('click', (ev) => {
        ...
        const el = target?.closest('[data-explain]');
        if (!el) return;
        ev.preventDefault();
        ev.stopPropagation();
 ...
 */
export function markExplainable(root: Document | Element = document): void {
