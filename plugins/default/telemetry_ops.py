...
async def record(ctx: PluginContext):
    ...
    if not out_tokens and ctx.response and hasattr(ctx.response, "body"):
        try:
            ...
            logger.debug(
                "Telemetry output token estimate fallback parse skipped", exc_info=True
            )
    ...
    await rotator._add_log(
        f"FINOPS: Request complete. {in_tokens + out_tokens} tokens consumed. Est. Cost: ${total_cost:.4f}",
        level="SYSTEM",
    )
