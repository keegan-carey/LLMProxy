...
def _mock_agent():
    ...
    agent.features = {"injection_guard": True, "language_guard": True, "link_sanitizer": True}
...
async def test_guards_status_contract(app_agent):
    ...
    for field in ("enabled", "disabled_reason", "total_scanned", "total_blocked",
                  "block_by_signature", "signatures_count"):
        assert field in fw, f"firewall.{field} missing"

    assert "features" in data, "explain.js reads data.features for guard cards"
    assert isinstance(data["features"], dict)

    assert "circuit_breakers" in data, "explain.js reads circuit_breakers[<id>] for circuit kind"
...
async def test_registry_list_contract(app_agent):
    ...
    await agent.store.add_endpoint(LLMEndpoint(
    ...
    assert len(items) >= 1
    row = items[0]
    for field in ("id", "name", "url", "status", "latency", "priority", "type",
                  "circuit_state", "failure_count", "failure_threshold"):
...
async def test_models_list_contract(app_agent):
    ...
    for m in data["data"]:
        ...
        assert "owned_by" in m, "model.owned_by required — drilldown model kind groups by this"
        assert "object" in m
...
async def test_models_include_config_endpoints(app_agent):
    ...
    for expected in ("gpt-4o", "gpt-4o-mini", "llama3.3"):
        assert expected in ids, f"{expected} missing from /v1/models (config → UI inventory broken)"
...
async def test_plugins_list_shape(app_agent):
    ...
    items = data if isinstance(data, list) else (data.get("plugins") or data.get("data") or [])
